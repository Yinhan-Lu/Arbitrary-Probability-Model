#!/bin/bash
# ==========================================================================
# CONDITIONING PERCENTAGE SWEEP EXPERIMENTS
# ==========================================================================
# Systematic study of conditioning percentage effects on arbitrary
...
#     - 40-60%, 60-80%
#
# Models: Conditional, SigmaGPT-temporal, SigmaGPT-scramble
# Total: 6 configs × 3 models = 18 experiments
#
# Usage:
#   ./scripts/submit_cond_pct_sweep.sh           # Submit all experiments
#   ./scripts/submit_cond_pct_sweep.sh --dry-run # Show what would be submitted
#   ./scripts/submit_cond_pct_sweep.sh --skip-baseline # Skip 0-40% (already done)
# ==========================================================================
...
# Define model types
# Format: "model_type ordering_mode exp_suffix"
declare -a MODELS=(
    "conditional none cond"
    "sigmagpt temporal sgpt_temp"
    "sigmagpt random_scramble sgpt_scram"
)
...
echo "========================================="
echo "CONDITIONING PERCENTAGE SWEEP"
echo "========================================="
echo "Configurations: ${#CONFIGS[@]} conditioning ranges"
echo "Models: ${#MODELS[@]} model types"
echo "Total experiments: $((${#CONFIGS[@]} * ${#MODELS[@]}))"
...
for config in "${CONFIGS[@]}"; do
    ...
    for model in "${MODELS[@]}"; do
        read -r MODEL_TYPE ORDERING EXP_SUFFIX <<< "$model"

        # Construct experiment name
        EXP_NAME="sweep_${LABEL}_${EXP_SUFFIX}"

        # Construct job name (max 15 chars for SLURM)
        JOB_NAME="${LABEL:0:10}_${EXP_SUFFIX:0:4}"

        echo "----------------------------------------"
        echo "Experiment: $EXP_NAME"
        echo "  Model: $MODEL_TYPE"
        if [ "$MODEL_TYPE" == "sigmagpt" ]; then
...
# ==========================================================================
# Experiment: ${EXP_NAME}
# Model: ${MODEL_TYPE}, Ordering: ${ORDERING}
...
EOF
        ...
        # Add model-specific training command
...
if [ "$DRY_RUN" == "true" ]; then
    ...
    echo "Would submit: $((${#CONFIGS[@]} * ${#MODELS[@]} - SKIPPED)) jobs"
else
...
echo "  python utils/plot_comparison_metrics.py experiments/sweep_*"
