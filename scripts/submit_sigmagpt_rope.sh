#!/bin/bash
# ==========================================================================
# SIGMAGPT ROPE ABLATION STUDY: Model Size x Conditioning % x Ordering
# ==========================================================================
# Three-dimensional ablation study for SigmaGPT with RoPE:
...
#
# Key settings:
#   - position_encoding_type = "rope" (-> dual_rope internally)
#   - sigmagpt_mode = fair (~40% learning efficiency)
#   - min_conditioning = 0 (always)
#   - eval_pct = 1.0 (all non-cond = eval)
#
# Total: 3 sizes x 5 cond_pct x 2 orderings = 30 experiments
#
# Usage:
#   ./scripts/submit_sigmagpt_rope.sh                    # Submit all experiments
#   ./scripts/submit_sigmagpt_rope.sh --dry-run          # Show what would be submitted
#   ./scripts/submit_sigmagpt_rope.sh --model gpt2       # Submit only gpt2 model
#   ./scripts/submit_sigmagpt_rope.sh --cond 0.4         # Submit only 0-40% cond
#   ./scripts/submit_sigmagpt_rope.sh --ordering temporal # Submit only temporal ordering
# ==========================================================================
...
# mode2 boundary matches training range: 0-x% (in-distribution evaluation)
...
# Common training parameters (aligned with conditional script)
BATCH_SIZE=8
GRAD_ACCUM=16
...
for MODEL_CONFIG in "${MODEL_CONFIGS[@]}"; do
    ...
    for ORDERING in "${ORDERING_MODES[@]}"; do
        ...
        for config in "${COND_CONFIGS[@]}"; do
            ...
            JOB_NAME="sr${ORDER_SHORT}_c${COND_PCT}_${MODEL_CONFIG:0:4}"
...
# ==========================================================================
# SIGMAGPT ROPE EXPERIMENT
# Model: ${MODEL_CONFIG}
# Ordering: ${ORDERING}
# Conditioning: 0.0-${COND_MAX}
# Position Encoding: RoPE (dual-axis)
# ==========================================================================
