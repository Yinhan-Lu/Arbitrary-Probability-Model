"""
Sigma GPT Model (OLD VERSION - Double Position Encoding)

NOTE: This is the ORIGINAL Sigma GPT implementation based on nanoGPT style.
      It uses DOUBLE POSITION ENCODING (n_embd // 2 per position).
      The current implementation (sigmagpt_from_baseline.py) uses standard position encoding.

This file is kept for comparison and testing purposes.
...
class SigmaGPTOld(nn.Module):
    """
    Sigma GPT model with double position encoding (OLD VERSION)
    ...
    def __init__(self, config: GPT2Config):
        ...
        print(f"SigmaGPTOld (Double Position Encoding) initialized with {self.get_num_params() / 1e6:.2f}M parameters")
